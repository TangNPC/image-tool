...
DEFAULT_BASE_URL = os.environ.get("IMAGE_TOOLS_DEFAULT_BASE_URL", "").strip().rstrip("/")
DEFAULT_IMAGE_GROUP = os.environ.get("IMAGE_TOOLS_DEFAULT_IMAGE_GROUP", "").strip()
DEFAULT_IMAGE_MODEL = os.environ.get("IMAGE_TOOLS_DEFAULT_IMAGE_MODEL", "").strip()
DEFAULT_IMAGE_TOKEN_NAME = os.environ.get("IMAGE_TOOLS_DEFAULT_IMAGE_TOKEN_NAME", "").strip()
DEFAULT_CODEX_GROUP = os.environ.get("IMAGE_TOOLS_DEFAULT_CODEX_GROUP", "").strip()
DEFAULT_CODEX_MODEL = os.environ.get("IMAGE_TOOLS_DEFAULT_CODEX_MODEL", "").strip()
DEFAULT_CODEX_TOKEN_NAME = os.environ.get("IMAGE_TOOLS_DEFAULT_CODEX_TOKEN_NAME", "").strip()
...
OPENAI_TEXT_PROXY_PATHS = {
    "/api/openai/v1/models": "/v1/models",
    "/api/openai/v1/chat/completions": "/v1/chat/completions",
    "/api/openai/v1/responses": "/v1/responses",
}
...
def normalize_new_api_base_url(value: str) -> str:
    ...
    if parsed.scheme != "https" or not parsed.netloc:
        raise NewApiError("Base URL 必须是有效的 https 地址")
    if parsed.username or parsed.password:
        raise NewApiError("Base URL 不能包含账号密码")
    if not is_public_hostname(parsed.hostname):
        raise NewApiError("Base URL 必须是可公开访问的地址")
...
def proxy_openai_text_request(
    ...
    try:
        with open_url(request, timeout=REQUEST_TIMEOUT) as response:
            ...
            return response.status, response_body, response_content_type
        ...
        return exc.code, response_body, response_content_type
    except urllib.error.URLError as exc:
        raise OpenAIProxyError(f"无法连接上游 API：{exc.reason}") from exc
...
def obtain_managed_key(
    base_url: str,
    username: str,
    password: str,
    image_group: str,
    image_model: str,
    image_token_name: str,
    codex_group: str,
    codex_model: str,
    codex_token_name: str,
) -> dict[str, Any]:
    if not username.strip() or not password:
        raise NewApiError("请输入账号和密码")
    if not image_group.strip() or not image_model.strip() or not image_token_name.strip():
        raise NewApiError("请完整填写生图 Token 配置")
    if not codex_group.strip() or not codex_model.strip() or not codex_token_name.strip():
        raise NewApiError("请完整填写文本 Token 配置")
    ...
    image_key = session.obtain_token_key(
        image_token_name.strip(),
        image_group.strip(),
        image_model.strip(),
    )
    codex_key = session.obtain_token_key(
        codex_token_name.strip(),
        codex_group.strip(),
        codex_model.strip(),
    )
...
class ImageToolsHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed_path.path in OPENAI_TEXT_PROXY_PATHS:
            self.handle_openai_text_proxy(parsed_path, "GET")
            return
    ...
    def do_POST(self) -> None:
        ...
        if parsed_path.path in OPENAI_IMAGE_PROXY_PATHS:
            ...
            return
        if parsed_path.path in OPENAI_TEXT_PROXY_PATHS:
            self.handle_openai_text_proxy(parsed_path, "POST")
            return
        ...
        try:
            ...
            result = obtain_managed_key(
                ...
                str(payload.get("imageGroup") or DEFAULT_IMAGE_GROUP),
                str(payload.get("imageModel") or DEFAULT_IMAGE_MODEL),
                str(payload.get("imageTokenName") or DEFAULT_IMAGE_TOKEN_NAME),
                str(payload.get("codexGroup") or DEFAULT_CODEX_GROUP),
                str(payload.get("codexModel") or DEFAULT_CODEX_MODEL),
                str(payload.get("codexTokenName") or DEFAULT_CODEX_TOKEN_NAME),
            )
    ...
    def handle_openai_text_proxy(self, parsed_path: urllib.parse.ParseResult, method: str) -> None:
        try:
            query = urllib.parse.parse_qs(parsed_path.query)
            base_url = normalize_public_base_url((query.get("base_url") or [""])[0])
            upstream_path = OPENAI_TEXT_PROXY_PATHS[parsed_path.path]
            auth_header = self.headers.get("Authorization", "")
            if not auth_header.startswith("Bearer "):
                raise OpenAIProxyError("缺少 API Key")
            ...
            status, response_body, response_content_type = proxy_openai_text_request(
                base_url=base_url,
                upstream_path=upstream_path,
                method=method,
            ...
            proxy_response(self, status, response_body, response_content_type)
        except OpenAIProxyError as exc:
            json_response(
                self,
                HTTPStatus.BAD_GATEWAY,
