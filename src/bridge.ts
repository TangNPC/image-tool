...
function openAiTextProxyUrl(path: 'models' | 'chat/completions' | 'responses', baseUrl: string) {
  return `/api/openai/v1/${path}?base_url=${encodeURIComponent(normalizeBaseUrl(baseUrl))}`
}
...
export const bridge: ImageApiClient = {
  ...
  async listModels(args) {
    const response = await fetch(openAiTextProxyUrl('models', args.baseUrl), {
      method: 'GET',
  ...
  async optimizePrompt(payload) {
    const response = await fetch(openAiTextProxyUrl('chat/completions', payload.baseUrl), {
      method: 'POST',
    ...
    try {
      ...
      const fallbackResponse = await fetch(openAiTextProxyUrl('responses', payload.baseUrl), {
        method: 'POST',
        headers: headers(payload.apiKey),
